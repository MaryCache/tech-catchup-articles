---
title: "『生成しないAI』Jevを触って、AIエージェントの「意味判断」を切り出せると気づいた"
emoji: "🧩"
type: "tech"
...
私のClaude Code環境には、Jevを知る前からすでに機械的な制御をかなり入れていました。危険な操作を `PreToolUse` hook で止めるガード、認証情報へのアクセス制限、Green / Yellow / Red の権限境界などです。

つまり、Jevを触って初めて「機械で止められるものは機械で止める」と思ったわけではありません。

むしろ今回見えてきたのは、その既存の構造の**ちょうど間に、これまで扱いづらかった層があった**ことでした。

> deterministicなコードでは書きにくい。けれど、生成LLMへ丸ごと任せるほどでもない。そんな「意味判断」だけを独立した部品にできるのではないか？

この記事では、Jevを実際に試した小規模な実験、TypeSafeコミュニティでの議論、コミュニティで共有されていた実装事例から、その「意味判断レイヤー」をどう扱えそうか整理します。
...
私の環境でも、破壊的操作や認証情報アクセスのように条件を機械的に書けるものはコードで止めていました。一方で、
...
のような、**ルールはあるが機械的な条件式へ落としにくい判断**は、結局Claude自身の自然言語理解へ残っていました。

Jevは制御フローそのものを実行するわけではありません。Jevが返すのは、**コード側が条件分岐に利用できる意味的な観測値**です。

ここが、私にとってJevの一番新しかった部分です。

---

## 3. すでに機械的制御は入れていた。Jevが埋めたのはその「間」だった

ここは自分のClaude Code環境を振り返ると、最初に書こうとしていた説明とは少し違っていました。

私はJevを知る前から、Claude Codeに対してかなり明示的なauthority boundaryを作っています。

たとえば、`settings.json` では `.env`、credentials、秘密鍵などへの `Read` をdenyしています。さらに `PreToolUse` hookからPython製のguardを呼び、Claudeが判断を誤っても一部の破壊的操作を**実行前にコードで止める**ようにしています。
...
Jevを触って変わったのは、「機械的制御の重要性に気づいた」ことではありません。

**自然言語ルールと生成LLMの間に、typedでprobabilisticなsemantic decision layerを挟めるかもしれないと気づいた**ことです。
...
Jevは、既存の機械的制御を置き換えるものというより、**そこでは表現しきれずLLM側へ残っていた意味判断を、さらに一段分離するための部品**として見えました。
...
私自身のClaude Code環境でも、自然言語の `AUTHORITY.md` を置く一方で、絶対禁止の一部はすでに `PreToolUse` hookのguardへ移しています。
...
ということ自体はJev以前からやっていました。

Bend2を見て面白いと思ったのは、その先です。数学的・論理的に形式化できる不変条件なら、単なるinstructionや通常コードだけでなく、**proofの対象へできる領域がさらにあるのではないか**という点でした。
...
Jevを触って、この部分もさらに分割できるのではないかと思うようになりました。
...
それでも、すでにある決定論的な制御とLLMの間へ、**「意味を読むが、自由生成も状態変更もしない判断層」**を置けるのはかなり面白い。

私にとってJevは「LLMに任せすぎていたと気づかせたモデル」というより、**すでにやっていた責務分離を、semantic decisionまで拡張できると気づかせたモデル**でした。
